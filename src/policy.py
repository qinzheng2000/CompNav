...
class NavNet(Net):
    ...
    def __init__(
        ...
        goal_observation_space = spaces.Dict(
            {"rgb": observation_space.spaces[self._goal_sensor_uuid]} if "feature" not in self._goal_sensor_uuid else {
                "feature": observation_space.spaces[self._goal_sensor_uuid]}
        )
    ...
    def __call__(self, observations):
        ...
        with torch.no_grad():
            ...
            for i in range(target_tensor.shape[0]):
                ...
                matched_point_features = -torch.ones((self.output_shape,), dtype=torch.float32,
